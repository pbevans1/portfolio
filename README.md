...
## Expected Data Files: 
* The project expects a tilde delimited .csv file called 'food_database.csv' to be in the directory 'data/'. 
* * The file is expected to be in the following format:  product id\~product name\~manufacturer\~energy units\~carb units\~fat units\~protein units\~serving size in grams or ml\~grams or ml\~serving size\~serving size units  
