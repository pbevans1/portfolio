...
## Expected Data Files: 
* The project expects a tilde delimited .csv file called 'food_database.csv' to be in the directory 'data/'. This file will be built by the makefile if you follow the recommended build procedure. If you wish to create a custom csv file, follow these guidelines. 
    * Place the file in the 'data/' directory
    * Name the file 'food_database.csv'
    * Write entries in the following format:  product id\~product name\~manufacturer\~energy units\~carb units\~fat units\~protein units\~serving size in grams or ml\~grams or ml\~serving size\~serving size units 
        * Example: 45001524\~MOCHI ICE CREAM BONBONS\~G. T. Japan, Inc.\~200.0\~35.0\~6.25\~2.5\~40.0\~g\~1.0\~PIECE
        * Lines that do not follow this format will be skipped or coerced to the correct format.
        * Numeric information may be lost when products are coerced to the correct format. Any losses will be zero filled.
    * Keep lines to 9000 characters or less
        * Lines longer than 9000 characters will be skipped
    
* User log files are generated by the program as necessary, and there should be no reason to create one by hand. If for some reason you do wish to create a log file manually, follow these guidelines:
    * Place the file in the 'data/' directory
    * Name the file {name}.log, where name is a string of lowercase letters
        * Example: preston.log
    * Use CSV format with \~ as the delimiter
    * Write entries as follows: Date (yyyy/mm/dd)\~Product Name\~NumServings 
        * Example: 2018/03/22\~APPLE BLOSSOMS\~3.00
        * Entries that fail to follow this format will be ignored
        * Entries that do not exactly match a product in the database will be ignored
    * Place entries in the file in descending order (by date, then by product)
        * 2018/03/22\~BAGELS\~1.20
        * 2018/03/22\~APPLE BLOSSOMS\~3.00
        * 2018/03/12\~TESSEMAE'S, HONEY MUSTARD\~44.00
    * Explanation of above: The diary is built at runtime by reading the file and inserting each entry into a sorted array. When the file is written by the program it is written in order, so each insert operation takes constant time and the total cost of reading is linear. Failure to follow the sorting guideline will cause the program to incur a one time O(n^2) cost to insertion sort the the diary. On exit, the program will automatically rewrite the file in sorted order if necessary.
...
- Don't worry if not all of these directories are present on download. The Makefile will create them as they're needed.
